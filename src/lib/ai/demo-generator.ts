...
import type { GenerateRequest, GenerateResponse, PlanRequest, PlanResponse } from "./contract";
...
const DEMO_STAGE_PACING_MS = 160;

type DemoPlan = { kind: DemoKind; wantsDark: boolean; wantsChart: boolean; isEdit: boolean };

type DemoRequest = { prompt: string; currentHtml?: string };

/** Curated templates only cover the three documented examples plus one expense edit. */
function resolveDemoPlan(request: DemoRequest): DemoPlan | null {
  ...
  return {
    kind,
    wantsDark: /深色|dark|夜间/.test(request.prompt.toLowerCase()),
    wantsChart: /图表|统计|chart|visual/.test(request.prompt.toLowerCase()),
    isEdit: Boolean(request.currentHtml),
  };
}

const DEMO_SPECS: Record<DemoKind, PlanResponse> = {
  expense: {
    goal: "记录每笔收支，并随时看到余额与分类构成。",
    coreFeatures: ["添加收入与支出记录", "按分类筛选流水", "实时计算余额与收支合计"],
    nonGoals: ["不接入银行或支付接口", "不做账号与云同步"],
    assumptions: ["使用人民币与本地货币格式", "数据只存在于当前页面，刷新后清空"],
    openQuestions: [],
  },
  pomodoro: {
    goal: "用一个番茄钟专注当前任务，并跟踪完成的专注次数。",
    coreFeatures: ["25 分钟倒计时与开始/暂停/重置", "添加并勾选任务", "统计完成的专注次数"],
    nonGoals: ["不做跨设备同步", "不做多人协作"],
    assumptions: ["固定 25 分钟专注时长", "任务列表只存在于当前页面"],
    openQuestions: [],
  },
  habit: {
    goal: "每天为习惯打卡，并看到连续坚持的天数。",
    coreFeatures: ["为多个习惯逐日打卡", "实时计算连续天数", "展示一周完成概览"],
    nonGoals: ["不做提醒推送", "不做账号与云同步"],
    assumptions: ["以自然周为周期", "打卡记录只存在于当前页面"],
    openQuestions: [],
  },
};

function demoName(plan: DemoPlan) {
  if (plan.kind === "expense") return "个人记账本";
  return plan.kind === "pomodoro" ? "专注番茄钟" : "每周习惯打卡";
}

function demoSummary(plan: DemoPlan) {
  if (plan.kind === "expense") {
    if (!plan.isEdit) return "已创建可添加收支记录并实时计算余额的个人记账应用。";
    return `已更新记账应用${plan.wantsDark ? "，切换为深色界面" : ""}${plan.wantsChart ? "并加入分类支出图表" : ""}。`;
  }
  return plan.kind === "pomodoro"
    ? "已创建带任务列表、开始/暂停/重置和完成次数统计的番茄钟。"
    : "已创建支持每日打卡、连续天数和一周进度的习惯追踪器。";
}

function demoHtml(plan: DemoPlan) {
  if (plan.kind === "expense") return expenseHtml(plan.wantsDark, plan.wantsChart);
  return plan.kind === "pomodoro" ? pomodoroHtml() : habitHtml();
}

function pause() {
  return new Promise((resolve) => setTimeout(resolve, DEMO_STAGE_PACING_MS));
}

/**
 * Phase one in Demo Mode: return the curated specification for a supported
 * example so the approval gate behaves exactly like the live path.
 */
export async function runDemoPlan(request: PlanRequest, report: StageReporter): Promise<PlanResponse> {
  report("planner", "active");
  await pause();

  const plan = resolveDemoPlan({ prompt: request.prompt });
  if (!plan) {
    report("planner", "failed");
    throw new BuildError("CONFIG_MISSING", UNSUPPORTED_MESSAGE);
  }

  report("planner", "completed");
  return DEMO_SPECS[plan.kind];
}

/**
 * Phase two in Demo Mode: build the curated template for an approved
 * specification. Stages map to the same real units of work as the live agent,
 * and results are always labelled `demo`.
 */
export async function runDemoBuild(request: GenerateRequest, report: StageReporter): Promise<GenerateResponse> {
  const plan = resolveDemoPlan(request);
  if (!plan) throw new BuildError("CONFIG_MISSING", UNSUPPORTED_MESSAGE);

  report("designer", "active");
  await pause();
  const summary = demoSummary(plan);
  report("designer", "completed");
  report("engineer", "active");

  await pause();
  const html = hardenGeneratedHtml(demoHtml(plan));
  ...
  if (!validation.ok) {
    report("reviewer", "failed");
    throw new BuildError("OUTPUT_INVALID", `内置模板未通过校验：${validation.reason}。`);
  }
  report("reviewer", "completed");

  return { projectName: demoName(plan), summary, html, mode: "demo" };
}
