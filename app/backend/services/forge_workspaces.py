...
# ------------------ Service Layer ------------------
class Forge_workspacesService:
    """Service layer for Forge_workspaces operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Forge_workspaces]:
        """Create a new forge_workspaces"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Forge_workspaces(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created forge_workspaces with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating forge_workspaces: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for forge_workspaces {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Forge_workspaces]:
        """Get forge_workspaces by ID (user can only see their own records)"""
        try:
            query = select(Forge_workspaces).where(Forge_workspaces.id == obj_id)
            if user_id:
                query = query.where(Forge_workspaces.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching forge_workspaces {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of forge_workspacess (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Forge_workspaces.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Forge_workspaces, field):
                        column = getattr(Forge_workspaces, field)
            ...
            stmt = select(Forge_workspaces, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Forge_workspaces, field_name):
                        stmt = stmt.order_by(getattr(Forge_workspaces, field_name).desc())
                else:
                    if hasattr(Forge_workspaces, sort):
                        stmt = stmt.order_by(getattr(Forge_workspaces, sort))
            else:
                stmt = stmt.order_by(Forge_workspaces.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Forge_workspaces)
                for condition in conditions:
            ...
            logger.error(f"Error fetching forge_workspaces list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Forge_workspaces]:
        """Update forge_workspaces (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Forge_workspaces {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated forge_workspaces {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating forge_workspaces {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete forge_workspaces (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Forge_workspaces {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted forge_workspaces {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting forge_workspaces {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Forge_workspaces]:
        """Get forge_workspaces by any field"""
        try:
            if not hasattr(Forge_workspaces, field_name):
                raise ValueError(f"Field {field_name} does not exist on Forge_workspaces")
            column = getattr(Forge_workspaces, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Forge_workspaces).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching forge_workspaces by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Forge_workspaces]:
        """Get list of forge_workspacess filtered by field"""
        try:
            if not hasattr(Forge_workspaces, field_name):
                raise ValueError(f"Field {field_name} does not exist on Forge_workspaces")
            column = getattr(Forge_workspaces, field_name)
            ...
            logger.error(f"Error fetching forge_workspacess by {field_name}: {str(e)}")
            raise